...
prediction_data_w <- simple_logistic_regression(baseline, "ffr_0.8", "ccta_mla_w")[[2]]
...
# Define test sets
test_set <- list(
  coords(roc_mla), coords(roc_mla_w),
  coords(roc_mla_ln), coords(roc_ostial),
  coords(roc_ostial_w), coords(roc_ostial_ln)
)

prediction_data <- list(
 prediction_data_mla, prediction_data_w,
prediction_data_mla_ln, prediction_data_ostial, 
prediction_data_ostial_w, prediction_data_ostial_ln
)

variables <- c(
  "CCTA MLA", "CCTA MLA width", "CCTA MLA luminal narrowing",
  "CCTA ostial area", "CCTA ostial width", "CCTA ostial luminal narrowing"
)
...
train_and_evaluate <- function(seed) {
  ...
  # Split the data into training (78%) and testing (22%) sets
...
write_xlsx(roc_summary_df_ffr0.8_rounded, "C:/WorkingData/Documents/2_Coding/Python/_team/statistical_analysis/data/roc_summary_ffr0.8_rounded_summary.xlsx")
